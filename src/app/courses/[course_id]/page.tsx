...
const Page = () => {
  ...
  const handleSubmit = async () => {
    ...
    const questions = selectedSectionData?.sectionContent[0]?.data?.questions;
    if (!questions) {
    ...
    const results = Object.entries(selectedAnswers)
      .map(([questionId, selectedIndex]) => {
        ...
        return {
          ...
          questionId,
          answer: selectedIndex !== null ? selectedIndex.toString() : "",
  ...
  return (
    <div className="flex pt-28 pb-16 px-20 w-full max-h-150">
      ...
      <div className="w-3/4 pl-8 max-h-120 overflow-y-auto overflow-x-hidden">
        {isPurchased === null ? (
          <p className="mt-4 text-gray-500">Checking purchase status...</p>
        ...
        {isPurchased && selectedSectionContent ? (
          <div className="">
            <div className="">
              {typeof selectedSectionContent === "string" ? (
              ...
              ) : selectedSectionContent[0].type.toLowerCase() === "lecture" ? (
                <div className="flex flex-col gap-6">
                  <h2 className="text-2xl font-semibold">
                    {selectedSectionData?.sectionTitle}
                    {" > "}
                    {selectedSectionContent[0].data.lectureTitle}
                  </h2>
                  {/* <pre>{JSON.stringify(selectedSectionContent, null, 2)}</pre> */}
                  <VideoComponent VideoContent={selectedSectionContent} />
                </div>
              ) : selectedSectionContent[0].type.toLowerCase() === "test" ? (
                <div className="flex flex-col gap-6">
                  <h2 className="text-2xl font-semibold">
                    {selectedSectionData?.sectionTitle}
                    {" > "}
                      ...
                      <div key={question?.questionId}>
                        ...
                        {question.questionType === "descriptive" && (
                          <div className="flex flex-col gap-4">
                            ...
                            {isTestSubmitted && (
                              <p className="text-sm">
                                {descriptiveResult?.similarity_score < 0.4
                                  ? "Your Answer: Incorrect"
                                  : descriptiveResult?.similarity_score < 0.7
                ...
                <div className="flex flex-col gap-6">
                  <h2 className="text-2xl font-semibold">
                    {selectedSectionData?.sectionTitle}
                    {" > "}
                    {selectedSectionContent[0].data.assignmentTitle}
                  </h2>
                  <pre>{JSON.stringify(selectedSectionContent, null, 2)}</pre>
                </div>
              )}
        ...
        {isPurchased && selectedSection && (
          <>
            <CommentSection comments={comments} userID={user?.id as string} />
            <CommentInput
              sectionId={selectedSection}
